...
const OUT = path.join(ROOT, 'src/app/company-codes/company-seo-content/backup-codes.ts');
...
const body = rows.map(r => `  '${esc(r.slug)}': { code: '${esc(r.backup.code)}', discount: '${esc(r.backup.discount)}' },`).join('\n');
const ts = `// AUTO-GENERATED by scripts/populate-v1-backup-codes.js — one-off (v1 is frozen).
// Fallback code per v1 (company-codes) shop, shown by CompanyCodesComponent when
// discounts.json has no live code for the shop — so the page renders its content
// instead of the "404 niet gevonden" shell. Waterfall: discounts.json most-recent
// -> diski.discounts most-recent -> synthesized ACTIE/OFF/SHOP+10/15.
export const V1_BACKUP_CODES: Record<string, { code: string; discount: string }> = {
${body}
};
`;
